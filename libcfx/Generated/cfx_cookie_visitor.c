...
int CEF_CALLBACK _cfx_cookie_visitor_release(struct _cef_base_ref_counted_t* base) {
    ...
    if(count == 0) {
        if(((cfx_cookie_visitor_t*)base)->wrapper_kind == 0) {
            cfx_gc_handle_switch(&((cfx_cookie_visitor_t*)base)->gc_handle, GC_HANDLE_FREE);
        } else {
            cfx_gc_handle_switch(&((cfx_cookie_visitor_t*)base)->gc_handle, GC_HANDLE_FREE | GC_HANDLE_REMOTE);
        }
        free(base);
        return 1;
    }
