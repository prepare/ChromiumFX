...
void CEF_CALLBACK _cfx_write_handler_add_ref(struct _cef_base_ref_counted_t* base) {
    InterlockedIncrement(&((cfx_write_handler_t*)base)->ref_count);
}
int CEF_CALLBACK _cfx_write_handler_release(struct _cef_base_ref_counted_t* base) {
    int count = InterlockedDecrement(&((cfx_write_handler_t*)base)->ref_count);
    if(count == 0) {
        if(((cfx_write_handler_t*)base)->wrapper_kind == 0) {
            cfx_gc_handle_switch(&((cfx_write_handler_t*)base)->gc_handle, GC_HANDLE_FREE);
        } else {
            cfx_gc_handle_switch(&((cfx_write_handler_t*)base)->gc_handle, GC_HANDLE_FREE | GC_HANDLE_REMOTE);
        }
        free(base);
        return 1;
    }
...
int CEF_CALLBACK _cfx_write_handler_has_one_ref(struct _cef_base_ref_counted_t* base) {
